	...
	methods: {
				...
				if (data.success_code == 200) {
					if (this._getType(data) == "object") {
						...
						if (this._getType(detail) == "object") {
							this.title = detail.hasOwnProperty("title") ? detail.title : ""; //标题名称
							this.tag = detail.hasOwnProperty("tag") ? detail.tag : ""; //标签名称
							this.image = detail.hasOwnProperty("image") ? detail.image : ""; //产品图片
							this.group = {
								amounts: detail.hasOwnProperty("amounts") ? detail.amounts : "", //名额
								price: detail.hasOwnProperty("price") ? detail.price : "", //价格
								applicant_num: detail.hasOwnProperty("applicant_num")
									? detail.applicant_num
									: "", //申请人数
								status: detail.hasOwnProperty("status") ? detail.status : "" //状态
							};
							if (detail.hasOwnProperty("item_detail")) {
								const item_detail = detail.item_detail;
								if (this._getType(item_detail) == "string") {
									// 图片懒加载处理
							...
							if (detail.hasOwnProperty("rule_detail")) {
							...
							if (detail.hasOwnProperty("applicants")) {
		...
		loadingList() {
			...
			if (pageIndex <= Object.keys(handle).length) {
				...
				if (this._getType(handle) == "object") {
