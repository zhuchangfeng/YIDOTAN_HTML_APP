<<<<<<< HEAD
<template>
	<div class="zhongce__id" v-loading.body="isLoading">
		<div class="zhongce__id--top">
			<div class="top-type type-1">{{tag}}</div>
			<div class="top-image">
				<img v-lazy="image" alt class="lazy" />
			</div>
		</div>

		<div class="zhongce__id--bottom">
			...
			<div class="bottom-group">
				...
				<div class="group-col">
					...
					<span v-else-if="group.status=='1'" class="zc-ing">申请中</span>
					<span v-else-if="group.status=='2'" class="zc-over">已结束</span>
					<span v-else-if="group.status=='0'" class="zc-ready">即将开始</span>
		...
		<img v-lazy="'https://h5.yidoutang.com/v4/public/zhongce.png'" class="lazy zhongce-image" alt />
		<!-- 商品详情 -->
		<div class="zhongce__id--info borderBottom">
			<i class="fa fa-shopping-cart" aria-hidden="true"></i> 商品详情
		</div>
		<div
			class="zhongce__id--part borderBottom"
			v-html="item_detail"
		...
		>
			...
			<div class="part-more borderTop borderBottomAfter" @click="loadingList">{{loadingText}}</div>
		</div>
		<div class="zhongce__id--part borderBottom" style="padding-left: 0;padding-right: 0;" v-else>
			<div class="part-info">暂时没人申请，快去抢鲜体验吧!</div>
		</div>
		<!-- 申请名单 end-->
	</div>
</template>
			...
			if (/^[0-9]*$/.test(params?.id)) {
				...
				if (data?.success_code === 200) {
					...
					if (this._getType(detail) === 'object') {
						...
						this.group = {
							amounts: detail?.quota ? detail.quota : '', // 名额
						...
						if (typeof item_detail === 'string') {
							...
							const contentp = item_detail.replace(regexp1, function(
								match,
								p1,
								p2,
								p3
						...
						if (this._getType(rule_detail) === 'string') {
							// 图片懒加载处理
							const regexp1 = new RegExp(/(<img.*?src=")(.*?)(".*?\/?)>/, 'g')
							const contentp = rule_detail.replace(regexp1, function(
								match,
								p1,
								p2,
								p3
