<template>
  <div class="new-id" v-loading.body="isLoading" ref="new">
      ...
      <div class="new-author">
        <div class="author-l">
          <div class="author-avatar">
            ...
            <img
              src="https://h5.yidoutang.com/v4/public/identity_2.png"
              alt
    ...
    <div class="house-artBd">
      <div class="house-artPart" ref="artBd">
      ...
      >
        ...
        <div v-for="(item2, index2) in item" :key="index2">
          <div
            class="partImg"
            v-if="item2.match && item2.match.image"
            :style="{ width: item2.width + 'px', height: item2.height + 'px' }"
          >
...
export default {
	...
	methods: {
				...
				try {
					...
					if (success_code === 200) {
						if (this._getType(data) === 'object') {
							...
							if (spaces) {
								this.$nextTick(() => {
									const { width } = this.$refs.artBd.getClientRects()[0]
				...
				if (this._getType(this.$refs[dom]) === 'array') {
					if (this.$refs[dom].length > 0) {
						const timer = setTimeout(() => {
							this.$refs[dom][0].scrollIntoView()
						})
		...
		getHash() {
			...
			if (!isEmpty(hash)) {
				const decodeHash = decodeURIComponent(hash).replace('#', '')
				this.$nextTick(() => {
